...
def _writer_format_instructions(report_format: str) -> str:
    rf = (report_format or 'markdown').strip().lower()
    if rf == 'bullets':
        return (
            '**Report format — bullet-led:** Use nested markdown bullet lists as the main structure under each section. '
            'Keep narrative paragraphs short; prefer bullets and sub-bullets for facts, still with [n] citations where required.'
        )
    if rf == 'executive_only':
        return (
            '**Report format — executive summary only:** Output ONLY `# Title` (one line) and `## Executive Summary` (2–4 tight paragraphs). '
            'Do NOT include separate ## Key Findings, ## Detailed Analysis, or ## Conclusion sections. '
            'Embed essential [n] citations inside the summary when sources exist. '
            'Place `## References` (or a short `### Key sources` with up to 5 APA one-liners) immediately after the Executive Summary when the catalog below requires citations — do not invent a ## Conclusion section just to satisfy ordering.'
        )
    if rf == 'full_detailed':
        return (
            '**Report format — full detail:** Use the same section headings as the default, but make `## Detailed Analysis` substantially deeper: '
            'multiple `###` subheadings, trade-offs, caveats, and implications while staying grounded in the analysis and cited sources only.'
        )
    return (
        '**Report format — markdown (default):** Use clear markdown: `# Title`, `## Executive Summary`, `## Key Findings`, '
        '`## Detailed Analysis`, `## Conclusion` with a balance of prose and lists where helpful.'
    )
...
def writer_agent_node(state: AgentState) -> AgentState:
    ...
    try:
        ...
        fmt_line = _writer_format_instructions(state.get('report_format') or 'markdown')
        response = llm.invoke([
            SystemMessage(content=(
                'You are a professional report writer. '
                + fmt_line
                + ' Then apply the citation and reference rules below. '
                + citation_rules
            )),
            HumanMessage(
                content=(
        ...
        return {
            **state,
            **append_step_duration(state, 'writer_agent', t0),
